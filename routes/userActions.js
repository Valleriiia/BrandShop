...
const userActions = require('../controllers/userActionsController'); // Тепер він містить всі функції кошика
const bcrypt = require('bcryptjs');
const pool = require('../models/db');
const jwt = require('jsonwebtoken');
const cardController = require('../controllers/cardController'); // Залишаємо його, якщо він потрібен для роутів картки
...
const isAuthenticated = (req, res, next) => {
    ...
    try {
        ...
        // Можливо, також передається об'єкт user, як у auth.js:
...
console.log('[DEBUG-ROUTES] userActions.js завантажено.'); // <--- ДОДАЙТЕ ЦЕ
console.log('[DEBUG-ROUTES] Перевіряємо наявність getUserOrders:', typeof userActions.getUserOrders); // <--- ДОДАЙТЕ ЦЕ


// ==== Улюблені ====
router.post('/favorites', isAuthenticated, userActions.addToFavorites);
router.delete('/favorites', isAuthenticated, userActions.removeFromFavorites);
router.get('/favorites/:userId', isAuthenticated, userActions.getFavorites);

// ==== Роути для кошика (БЕЗ ПРЕФІКСІВ /cart, оскільки вони будуть додані в app.js) ====
// router.post('/cart/add', isAuthenticated, userActions.addToCart); // ЦЕЙ БУДЕ ЗМІНЕНО
router.post('/cart/add', isAuthenticated, userActions.addToCart); // Змінено на /add
router.get('/cart', isAuthenticated, userActions.getCartItems); // Змінено на /
router.put('/cart/update-quantity', isAuthenticated, userActions.updateCartItemQuantity); // Змінено на /update-quantity
router.delete('/cart/remove/:productId', isAuthenticated, userActions.removeCartItem); // Змінено на /remove/:productId

// ==== Роут для акаунта (якщо він існує в userActionsController) ====
// Додайте цей роут, якщо ви очікуєте /api/v1/user/account
// Якщо ваша функція називається getAccountData в контролері:
// router.get('/account', isAuthenticated, userActions.getAccountData);
...
// Експорт об'єкта, який містить роутер та isAuthenticated
module.exports = {
    router: router,
